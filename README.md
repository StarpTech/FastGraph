...
In order to use option `2` and `3` you have to put your schema in `schema.graphql`. The schema is built and injected at build-time into the worker script. This is the only solution to provide constant good latency. The schema size is limited to approximately `800KB` after normalization and compression. The worker script size is itself [limited](https://developers.cloudflare.com/workers/platform/limits) to `1MB` from cloudflare. **Don't forget to validate your schema before the deployment.**
